...
def run_averaged_perceptron(training_df, r, T):
    w = np.zeros(len(training_df.columns))
    a = np.zeros(len(training_df.columns))
...
def test_w_with_dataframe(test_df, a):
    #change all of the 0 labels to -1
    ...
    return test_w(feature_matrix, labels, a)


"""returns the percentage error against the given feature matrix and labels"""
def test_w(feature_matrix, labels, a):
    num_wrong = 0
    for i in range(len(labels)):
        guess = np.matmul(a,feature_matrix[i])  # guess = w^T*x
        if(not np.sign(guess) == labels[i]):
            num_wrong += 1
